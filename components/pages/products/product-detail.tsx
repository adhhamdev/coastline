...
export default function ProductDetail({ product, similarProducts, currentUser }: ProductDetailProps) {
    ...
    const handleContact = async () => {
        ...
        const { data: chat } = await supabase
            ...
            .or(`user1_id.eq.${product.profiles.id},user2_id.eq.${product.profiles.id}`)
            .single();
                ...
                <div className="grid grid-cols-4 gap-4">
                    {product.images.map((image: string, index: number) => (
                        <button
                            ...
                            className={`relative aspect-square overflow-hidden rounded-lg border-2 ${selectedImage === index ? 'border-emerald-600' : 'border-transparent'
                                }`}
                        >
                            <Image
                                src={image}
            ...
            <div className="space-y-6">
                <div>
                    ...
                    <div className="mt-2 flex items-center space-x-2">
                        <Badge variant={product.category === 'gems' ? 'default' : 'secondary'}>
                            {product.category}
                        </Badge>
                ...
                <Card className="p-4">
                    <div className="flex items-start justify-between">
                        <Link href={`/${product.profiles.username}`} className="flex items-center space-x-3">
                            ...
                            <div>
                                <div className="flex items-center space-x-2">
                                    <span className="font-semibold">{product.profiles.username}</span>
                                ...
                                <p className="text-sm text-muted-foreground">{product.profiles.business_type}</p>
                            </div>
                        </Link>
                        <div className="flex space-x-2">
                            <Button variant="outline" size="icon" onClick={() => setIsSaved(!isSaved)}>
                                <Bookmark className={`h-5 w-5 ${isSaved ? 'fill-current text-emerald-600' : ''}`} />
                            </Button>
